...
sub write_readme {
    my $target = File::Spec->catfile( @_ );
    open my $f, '>', $target or die "$! writing $target";
    $f->print( <<EOR );
Files and directories:

assembly_metadata.yml
  Contains various metadata, notes, and statistics about this
  assembly.

membership.tab
  Tab-separated file showing the membership of preclusters, contigs,
  and reads.

phrap_consensus_all.fasta
  All consensus sequences (generated by Phrap) for assembled
  sequences.
...
generated_agp/
  AGP files generated from the Phrap assemblies for each chromosome.
